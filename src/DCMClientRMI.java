...
public class DCMClientRMI implements Serializable
{
    ...
    public ArrayList<RMIMessage>    inventoryServer(final long instanceParam, final Host hostParam, final int retentionTimeParam, final int timeoutParam, final int retryMaxParam, final boolean debugParam) { rmiMessageList.clear(); try { rmiMessageList = dcmServerRMI.inventoryServer(instanceParam, hostParam, retentionTimeParam, timeoutParam, retryMaxParam, debugParam); } catch (RemoteException ex) {  } return rmiMessageList; }
    ...
    public ArrayList<DCMPreset>       getDCMPresetList() { ArrayList<DCMPreset> dcmPresetList = new ArrayList<DCMPreset>(); try { dcmPresetList = dcmServerRMI.getDCMPresetList(); } catch (RemoteException ex) { dcmClient.log("Error: : RemoteException: getDCMPresetList " + ex.getMessage(), true, true, true); } return dcmPresetList; }
