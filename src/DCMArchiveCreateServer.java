...
public class DCMArchiveCreateServer implements ConfigurationCaller
{
    ...
    public DCMArchiveCreateServer(DCMInventoryServerServer inventoryReferenceServerParam, final Server server)
    {
        configuration = new Configuration(this);
        inventoryServerServerReference = inventoryReferenceServerParam;
...
//            public void run()
//            {
                if (!rrdHostDir.exists()) { rrdHostDir.mkdir(); log("Action:  DCMArchiveCreateServer: Creating missing directory: " + rrdHostDirString, false, true, true); };
                // Generic Properties
                ...
                for (Resource resource:server.getResourceList())
                {
                    ...
                    {
//                        log("Creating " + totalArchives + " archives for host: " + server.getHost().getHostname() + ". Please wait... (" + archiveCounter + "/" + totalArchives + ") "  + Math.round(archiveCounter / (totalArchives * 0.01)) + "%", true, false, false);
//                        log("Creating DS " + resource.getResource() + " Counter: " + resource.getCounterType() + " Archive: " + rrdHostDirString + resource.getRRDFile() + " at " + Tools.getHumanDate(server.getHost().getCreated()), true, true, true);
                        ...
                        try { rrdDef = new RrdDef(rrdHostDirString + resource.getRRDFile()); } catch (RrdException ex) { inventoryServerServerReference.rrdCreateFailureResponse(" Error: RrdException: RRDCreate: new RrdDef(resource.getRRDFile() " + ex.getMessage()); }
                        ...
                        try { rrdDb.close(); } catch (IOException ex) { inventoryServerServerReference.rrdCreateFailureResponse(" Error: IOException: RRDCreate: rrdDb.close() " + ex.getMessage()); }                                
                    }
//                    else
//                    {
...
//                    }
                ...
                log("Action:  DCMArchiveCreateServer: Creating " + totalArchives + " archives for host: " + server.getHost().getHostname() + " finished.", true, true, true);
//            }
//        });
...
//        rrdCreateThread.start();        
    ...
    {
        inventoryServerServerReference.log(messageParam, logToStatus, logApplication, logToFile);
    }
}
