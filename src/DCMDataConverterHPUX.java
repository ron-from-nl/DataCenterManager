...
public class DCMDataConverterHPUX // Being called by InventoryServer and PollServerto convert host & resources txt data to a server object(host,resources[]) and uses CommandsLibrary to get the right polling command for each resource
{
    //Server   server;
    Host     host;
    Resource resource;
    DCMCommandLibraryHPUX osCommands;
    int resCnt = 0;
    int pollServerInstance;
    int retentionTime;
//    String      headerKey;
    
    public DCMDataConverterHPUX(Host hostParam, int retentionTimeParam) throws UnknownHostException
    {
        //server = new Server();
        host = hostParam;
        osCommands = new DCMCommandLibraryHPUX(host);
 ...
 */
                id = resCnt; resCnt++; resourceType = "KBTransferedPerSec"; valueType  = "Reads"; pollCommand = osCommands.getDiskIOKBTransferedPerSecondCommand(resourceName); rrdFile = category + "_" + resourceType + "_" + resourceName + ".rrd";
                resource = new Resource(id,hostId,category,resourceType,valueType,counterType,resourceName,true,pollCommand,lastValue,warningLimit,criticalLimit,alertPolls,created,rrdFile,retentionTime);
                outputServer.addResource(resource);

                id = resCnt; resCnt++; resourceType = "mSecPerAverageSeek"; valueType  = "Writes"; pollCommand = osCommands.getDiskIOmSecPerAverageSeekCommand(resourceName); rrdFile = category + "_" + resourceType + "_" + resourceName + ".rrd";
                resource = new Resource(id,hostId,category,resourceType,valueType,counterType,resourceName,true,pollCommand,lastValue,warningLimit,criticalLimit,alertPolls,created,rrdFile,retentionTime);
                outputServer.addResource(resource);

                id = resCnt; resCnt++; resourceType = "SeeksPerSec"; valueType  = "KB"; pollCommand = osCommands.getDiskIOSeeksPerSecondCommand(resourceName); rrdFile = category + "_" + resourceType + "_" + resourceName + ".rrd";
                resource = new Resource(id,hostId,category,resourceType,valueType,counterType,resourceName,true,pollCommand,lastValue,warningLimit,criticalLimit,alertPolls,created,rrdFile,retentionTime);
                outputServer.addResource(resource);
            }
